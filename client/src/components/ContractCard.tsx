...
interface ContractCardProps {
  ...
  /** When true, uses tighter padding and narrower controls (e.g. in sidebar). */
  compact?: boolean;
}
...
export const ContractCard: React.FC<ContractCardProps> = ({
  ...
  const summaryText = typeof details.summary === 'string' ? details.summary : 'No summary.';
  const cardConclusion = details.conclusion?.trim() || (summaryText.length > 120 ? `${summaryText.substring(0, 120).replace(/\n/g, ' ')}...` : summaryText.replace(/\n/g, ' '));
  ...
  const deleteBtnStyle = { marginRight: -6 };
  ...
  return (
    <div style={cardStyleFinal} className="card-hover">
        ...
        <div style={{ marginTop: compact ? 8 : 10 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: compact ? 4 : 6 }}>
            {details.risk_flags.map((flag) => (
              <span key={flag} style={{ background: '#fef2f2', color: '#b91c1c', padding: compact ? '3px 8px' : '4px 10px', borderRadius: 8, fontSize: compact ? 11 : 12, fontWeight: 600 }}>
                {riskFlagLabel(flag)}
              </span>
            ))}
          </div>
          {'risk_flags_note' in details && details.risk_flags_note && (
            <p style={{ margin: '8px 0 0', fontSize: 12, color: '#64748b', lineHeight: 1.4 }}>{details.risk_flags_note}</p>
          )}
        </div>
      )}
      ...
      <div style={cardActionsFinal}>
        ...
        <button onClick={() => onViewInsights(summaryText)} style={S.secondaryBtn}>Insights</button>
        {isGoogleConnected && (
          <select
        ...
        <div style={{ position: 'relative', marginTop: 10 }}>
          ...
          {folderMenuOpen && (
            <>
              ...
              <div style={{ position: 'absolute', left: 0, right: 0, top: '100%', marginTop: 4, background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', zIndex: 11, padding: 6, maxHeight: 200, overflow: 'auto' }}>
                {customFolders.map((f) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '10px 12px', border: 'none', borderRadius: 8, background: inFolder ? `${f.color}20` : 'transparent', color: '#334155', fontSize: 14, cursor: 'pointer', textAlign: 'left' }}
                    >
                      <span>{inFolder ? '✓' : ''}</span>
                      <span>{f.symbol || '📁'}</span>
                      {f.name}
